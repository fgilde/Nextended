...
public class DtoGenerationSymbols
{
    ...
    public string GetBaseTypeString(INamedTypeSymbol type, Dictionary<string, INamedTypeSymbol> comTypeDict, bool asInterface)
    {
        return GetBaseTypeString(type, type.ClassCfg(this), comTypeDict, asInterface);
    }

    public string GetBaseTypeString(INamedTypeSymbol type, AutoGenerateDtoAttribute classAttr, Dictionary<string, INamedTypeSymbol> comTypeDict, bool asInterface)
    {
        return GetBaseTypeString(type, classAttr.Interfaces, comTypeDict, asInterface);
    }
    public string GetBaseTypeString(INamedTypeSymbol type, string[]? interfaces, Dictionary<string, INamedTypeSymbol> comTypeDict, bool asInterface)
    {
        var baseTypeStr = interfaces?.Any() ?? false ? $":{string.Join(", ", interfaces)}" : "";
        return GetBaseTypeString(type, baseTypeStr, comTypeDict, asInterface);
    }

    public string GetBaseTypeString(INamedTypeSymbol type, string baseTypeStr, Dictionary<string, INamedTypeSymbol> comTypeDict, bool asInterface)
    {
        var baseType = type.BaseType;
        if (baseType != null && comTypeDict.ContainsKey(baseType.ToDisplayString()))
        {
    ...
    public string GetNamespaceForProperty(IPropertySymbol property, Dictionary<string, INamedTypeSymbol> comTypeDict)
    {
        //var type = comTypeDict.ContainsKey(property.Type.ToDisplayString())
        //    ? comTypeDict[property.Type.ToDisplayString()]
        //    : null;
        //return GetNamespacePrefixString(Namespace(type));
        return GetNamespacePrefixString(GetClassCfgForType(property.Type, comTypeDict));
    }
    ...
    public AutoGenerateDtoAttribute? GetClassCfgForType(ITypeSymbol type, Dictionary<string, INamedTypeSymbol> comTypeDict)
    {
        return comTypeDict.ContainsKey(type.ToDisplayString()) 
            ? comTypeDict[type.ToDisplayString()].ClassCfg(this) 
            : null;
    }

    public bool PropertyInBaseType(IPropertySymbol prop, INamedTypeSymbol? baseType, Dictionary<string, INamedTypeSymbol> comTypeDict)
    {
        while (baseType != null && comTypeDict.ContainsKey(baseType.ToDisplayString()))
        {
