...
public class DtoCodeGenerator
{
    ...
    public DtoGeneratedFile GenerateAllForType(INamedTypeSymbol type, List<INamedTypeSymbol> allTypes)
    {
        ...
        var ns = autoGenAttr.Namespace ?? "TODO: DEFAULT"; // TODO:
    ...
    public DtoGeneratedFile? GenerateMappingExtensions(List<INamedTypeSymbol> types)
    {
        ...
        using (new ClassScope(sb, "MappingExtensions", ns))
        {
            ...
            foreach (var type in types)
            {
                ...
                if (isEnum)
                {
                    sb.AppendLine($"\t\t{autoGenAttr.ClassModifier.ToCSharpKeyword()} static {comTypeName}? {toComMethodName}(this {netTypeName}? src) => src.HasValue ? ({comTypeName})(int)src.Value : null;");
                    sb.AppendLine($"\t\t{autoGenAttr.ClassModifier.ToCSharpKeyword()} static {netTypeName}? {toNetMethodName}(this {comTypeName}? src) => src.HasValue ? ({netTypeName})(int)src.Value : null;");
                    sb.AppendLine($"\t\t{autoGenAttr.ClassModifier.ToCSharpKeyword()} static {comTypeName} {toComMethodName}(this {netTypeName} src) => ({comTypeName})(int)src;");
                    sb.AppendLine($"\t\t{autoGenAttr.ClassModifier.ToCSharpKeyword()} static {netTypeName} {toNetMethodName}(this {comTypeName} src) => ({netTypeName})(int)src;");
                    continue;
                }
                ...
                sb.AppendLine($"\t\t{autoGenAttr.ClassModifier.ToCSharpKeyword()} static {comTypeName}{genericParams} {toComMethodName}{genericParams}(this {netTypeName} src) {genericConstraints}");
                sb.AppendLine("\t\t{");
                ...
                sb.AppendLine($"\t\t{autoGenAttr.ClassModifier.ToCSharpKeyword()} static {netTypeName} {toNetMethodName}{genericParams}(this {comTypeName}{genericParams} src) {genericConstraints}");
                sb.AppendLine("\t\t{");
                sb.AppendLine("\t\t\tif(src == null) return null;");
        ...
        bool addGeneric)
    {
        ...
        foreach (var prop in DtoGenerationSymbols.GetDtoProperties(type, _symbols.Ignore))
        {
            ...
            sb.AppendLine($"\t\t{propTypeString} {propName} {{ {getterSetter} }}");
        }
        sb.AppendLine("\t}\n");
        return sb.ToString();
    }
