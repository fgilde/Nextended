...
public class DtoCodeGenerator
{
    ...
    private string GeneratePropertyMappingAssignment(IPropertySymbol prop, Dictionary<string, INamedTypeSymbol> comTypeDict, bool reverse)
    {
        ...
        string GetTargetType(ITypeSymbol typeSymbol, bool reverseDir)
        {
            ...
            {
                var autoGenAttr = nts.GetAttributeInstance<AutoGenerateDtoAttribute>(_symbols.AutoGenerateDto);
                return !reverseDir 
                    ? $"{DtoGenerationSymbols.GetComClassName(nts, autoGenAttr, false)}{DtoGenerationSymbols.GetGenericTypeParameters(nts)}" 
        ...
        if (DtoGenerationSymbols.IsComEnumType(netPropType, comTypeDict))
        {
            ...
            var enumToComMethod = !reverse
                ? DtoGenerationSymbols.GetToComMethodName(targetClass, targetAttr)
                : DtoGenerationSymbols.GetToNetMethodName(targetClass, targetAttr);
        ...
        if (DtoGenerationSymbols.IsComType(netPropType, comTypeDict))
        {
        ...
        using (new ClassScope(sb, "MappingExtensions", ns))
        {
            ...
            foreach (var type in types)
            {
                ...
                if(!autoGenAttr.GenerateMapping)
                    continue;
                // Wenn allerdings ein anderer typ eine property dieses Typs hat, muss dann statt der Map methode auch Prop.MapTo<TargetType> (siehe todo unten) benutzt werden.


                var toComMethodName = DtoGenerationSymbols.GetToComMethodName(type, autoGenAttr);
                var toNetMethodName = DtoGenerationSymbols.GetToNetMethodName(type, autoGenAttr);
                ...
                if (isEnum)
                {
                    ...
                    sb.AppendLine($"\t\tpublic static {netTypeName}? {toNetMethodName}(this {comTypeName}? src) => src.HasValue ? ({netTypeName})(int)src.Value : null;");
                    sb.AppendLine($"\t\tpublic static {comTypeName} {toComMethodName}(this {netTypeName} src) => ({comTypeName})(int)src;");
                    sb.AppendLine($"\t\tpublic static {netTypeName} {toNetMethodName}(this {comTypeName} src) => ({netTypeName})(int)src;");
                    continue;
                }
                ...
                sb.AppendLine($"\t\tpublic static {comTypeName}{genericParams} {toComMethodName}{genericParams}(this {netTypeName} src) {genericConstraints}");
                sb.AppendLine("\t\t{");
                ...
                // Reverse Mapping: DTO → Source (!!! Hier ist die Änderung !!!)
                sb.AppendLine($"\t\tpublic static {netTypeName} {toNetMethodName}{genericParams}(this {comTypeName}{genericParams} src) {genericConstraints}");
                sb.AppendLine("\t\t{");
                sb.AppendLine("\t\t\tif(src == null) return null;");
