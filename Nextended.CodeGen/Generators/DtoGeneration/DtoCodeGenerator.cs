...
public class DtoCodeGenerator
{
    ...
    public GeneratedFile GenerateAllForType(INamedTypeSymbol type, List<INamedTypeSymbol> allTypes)
    {
        ...
        return new GeneratedFile(fileName, ns, sb.ToString(), usings);
    }

    /// <summary>
    /// Methode für OneFilePerClass == false. Generiert eine Datei für alle Typen eines Namespaces.
    /// </summary>
    public GeneratedFile GenerateNamespaceFile(string ns, IEnumerable<INamedTypeSymbol> types, Dictionary<string, INamedTypeSymbol> comTypeDict)
    {
    ...
    public GeneratedFile? GenerateMappingExtensions(List<INamedTypeSymbol> types)
    {
        ...
        using (new ClassScope(sb, "MappingExtensions", ns))
        {
            ...
            foreach (var baseType in types.Select(type => type.BaseType))
            {
                ...
                if (comTypeDict.TryGetValue(baseTypeKey, out var baseTypeSymbol))
                {
            ...
            foreach (var type in types)
            {
                ...
                if (baseType != null && comTypeDict.TryGetValue(baseType.ToDisplayString(), out var baseTypeSymbol))
                {
                    var baseAutoGenAttr = baseTypeSymbol.ClassCfg(_symbols);
                    if (baseAutoGenAttr != null)
                    {
                ...
                foreach (var prop in comProps.Where(prop => !_symbols.PropertyInBaseType(prop, type.BaseType, comTypeDict)))
                {
                ...
                // --- Basismapping zurück
                if (baseType != null && comTypeDict.TryGetValue(baseType.ToDisplayString(), out var baseTypeSymbol2))
                {
        ...
        return new GeneratedFile("MappingExtensions.g.cs", ns, sb.ToString(), new[] { "System" });
        ...
        bool addGeneric)
    {
        ...
        var baseTypeStr = _symbols.GetBaseTypeString(type, classAttr, comTypeDict, true);
        ...
        foreach (var prop in DtoGenerationSymbols.GetDtoProperties(type, _symbols.Ignore))
        {
            ...
            var ns = _symbols.GetNamespaceForProperty(prop, comTypeDict);
        ...
        bool addGeneric)
    {
        ...
        var baseTypeStr = _symbols.GetBaseTypeString(type, [], comTypeDict, false)?.Replace(":", "");
