...
[![Downloads](https://img.shields.io/nuget/dt/Nextended.Web.svg)](https://www.nuget.org/packages/Nextended.Web/)
...
| **Generic controller** | `GenericODataController<T>` — query, key lookup and facet building already wired |
| **Faceted responses** | `FacetResourceSetSerializer` emits facet metadata alongside an OData result set |
| **Typed URLs** | `RequestHelper.UrlFor<TController>(…)`, `Action<TController>(…)`, `ActionLink<TController>(…)`, `RedirectToAction<TController>(…)` |
| **Downloads** | `controller.DownloadDataAsync(...)` for streams and writer callbacks, inline or attachment |
| **Detached work** | `BackgroundExecutor` — run work after the response, in a fresh DI scope, optionally against a captured request |
| **Uploads** | `IFormFile` and `IBrowserFile` extensions (`GetBytesAsync`, `GetReadableFileSize`, …) |
...
// refactor the action, the URL follows — no magic strings
```
...
Nothing is buffered — the callback writes straight to the response body.
...
`HttpContext.CaptureAsync(ct)` — for instance one taken before an earlier `await`.
...
Looking for permission-aware response shaping instead? That is
...
- [Nextended.EF](https://github.com/fgilde/Nextended/blob/main/Nextended.EF/README.md)
...
- **Nextended.Web** — ASP.NET Core utilities — zero-config OData (AddODataAuto), composable IQueryable OData appliers, strongly typed controller URLs, streaming download helpers and a background executor that can replay a captured request. _(this package)_
