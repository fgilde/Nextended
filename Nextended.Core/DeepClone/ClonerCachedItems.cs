...
{
    internal static class ClonerCachedItems
    {
        internal delegate object ObjectActivator();
        internal delegate object ObjectActivatorWithParameters(params object[] args);
        private static readonly SafeValueType<Type, SafeValueType<string, IClonerProperty>> CachedFields = new SafeValueType<Type, SafeValueType<string, IClonerProperty>>();
        private static readonly SafeValueType<Type, SafeValueType<string, IClonerProperty>> CachedPropertyInfo = new SafeValueType<Type, SafeValueType<string, IClonerProperty>>();
        ...
        private static readonly SafeValueType<Type, IClonerProperty> CachedFastDeepClonerIdentifier = new SafeValueType<Type, IClonerProperty>();
        private static readonly SafeValueType<Type, Type> CachedIListInternalTypes = new SafeValueType<Type, Type>();
        private static readonly CultureInfo Culture;

        static ClonerCachedItems()
        {
        ...
        internal static string GetFastDeepClonerIdentifier(this object o)
        {
            if (o == null)
                return null;
            var type = o.GetType();
            var p = CachedFastDeepClonerIdentifier.ContainsKey(type) ? CachedFastDeepClonerIdentifier[type] : CachedFastDeepClonerIdentifier.GetOrAdd(type, Cloner.GetFastDeepClonerProperties(type).FirstOrDefault(x => x.FastDeepClonerPrimaryIdentifire));
        ...
        internal static object ConvertToInterface(this Type interfaceType, object item)
        {
            ...
            if (interfaceType.IsAssignableFrom(type))
                return item;
            var props = Cloner.GetFastDeepClonerProperties(type);
            var args = new SafeValueType<string, object>();
            foreach (var iProp in Cloner.GetFastDeepClonerProperties(interfaceType))
            {
                var p = Cloner.GetProperty(type, iProp.Name);
                if (p == null)
                    continue;
                ...
                try
                {
                    if (iProp.PropertyType.IsInterface)
                        ...
                        args.TryAdd(iProp.Name, Convert.ChangeType(value, Cloner.GetProperty(interfaceType, p.Name).PropertyType));
            ...
            if (constructor == null)
                ...
                {
                    Cloner.GetProperty(newtype, p.Key).SetValue(returnValue, args[p.Key]);
                }
            return returnValue;
        }
        ...
        internal static IDictionary<string, IClonerProperty> GetFastDeepClonerFields(this Type primaryType)
        {
            if (!CachedFields.ContainsKey(primaryType))
            {
                var properties = new SafeValueType<string, IClonerProperty>();
                if (primaryType.GetTypeInfo().BaseType != null && primaryType.GetTypeInfo().BaseType.Name != "Object")
                {
                    primaryType.GetRuntimeFields().Where(x => properties.TryAdd(x.Name, new ClonerProperty(x))).ToList();
                    primaryType.GetTypeInfo().BaseType.GetRuntimeFields().Where(x => properties.TryAdd(x.Name, new ClonerProperty(x))).ToList();

                }
                else primaryType.GetRuntimeFields().Where(x => properties.TryAdd(x.Name, new ClonerProperty(x))).ToList();
        ...
        internal static IDictionary<string, IClonerProperty> GetFastDeepClonerProperties(this Type primaryType)
        {
            if (!CachedPropertyInfo.ContainsKey(primaryType))
            {
                ...
                if (primaryType.GetTypeInfo().BaseType != null && primaryType.GetTypeInfo().BaseType.Name != "Object")
                {
                    primaryType.GetRuntimeProperties().Where(x => properties.TryAdd(x.Name, new ClonerProperty(x))).ToList();
                    primaryType.GetTypeInfo().BaseType.GetRuntimeProperties().Where(x => properties.TryAdd(x.Name, new ClonerProperty(x))).ToList();
                }
                else primaryType.GetRuntimeProperties().Where(x => properties.TryAdd(x.Name, new ClonerProperty(x))).ToList();
