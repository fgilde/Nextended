...
namespace Nextended.Core.DeepClone
{
    internal class ReferenceClone
    {
        ...
        private readonly ClonerSettings _settings;

        internal ReferenceClone(FieldType fieldType)
        {
            _settings = new ClonerSettings() { FieldType = fieldType };

        }

        internal ReferenceClone(ClonerSettings settings)
        {
            if (settings != null)
                _settings = settings;
            else _settings = new ClonerSettings() { FieldType = FieldType.PropertyInfo };
        }
        ...
        internal object CloneTo(object itemToClone, object CloneToItem)
        {
            ...
            var props2 = ClonerCachedItems.GetFastDeepClonerProperties(type2);
            foreach (var prop2 in props2)
            {
                ...
                foreach (var attr in prop2.Value.GetCustomAttributes<ClonerColumn>())
                {
                ...
                if (prop != null)
                {
                    ...
                    {
                        prop2.Value.SetValue(CloneToItem, ClonerCachedItems.Value(value, prop2.Value.PropertyType, true));
                    }
                    else if (prop.PropertyType == prop2.Value.PropertyType)
                        prop2.Value.SetValue(CloneToItem, value.CloneDeep());
        ...
        private object ReferenceTypeClone(IDictionary<string, IClonerProperty> properties, Type primaryType, object objectToBeCloned, object appendToValue = null)
        {
        ...
        internal object Clone(object objectToBeCloned)
        {
            ...
            if (primaryType.IsArray || (objectToBeCloned as IList) != null)
            {
                ...
                foreach (var prop in ClonerCachedItems.GetFastDeepClonerProperties(primaryType).Where(x => !ClonerCachedItems.GetFastDeepClonerProperties(typeof(List<string>)).Any(a => a.Key == x.Key)))
                {
            ...
            else if (primaryType.IsAnonymousType()) // dynamic types
            {
                ...
                var props = ClonerCachedItems.GetFastDeepClonerProperties(primaryType);
            ...
            {
                resObject = ReferenceTypeClone((_settings.FieldType == FieldType.FieldInfo ? ClonerCachedItems.GetFastDeepClonerFields(primaryType) : ClonerCachedItems.GetFastDeepClonerProperties(primaryType)), primaryType, objectToBeCloned);
                if (_settings.FieldType == FieldType.Both)
                    resObject = ReferenceTypeClone(ClonerCachedItems.GetFastDeepClonerFields(primaryType).Values.ToList().Where(x => !ClonerCachedItems.GetFastDeepClonerProperties(primaryType).ContainsKey(x.Name)).ToDictionary(x => x.Name, x => x), primaryType, objectToBeCloned, resObject);
            }
