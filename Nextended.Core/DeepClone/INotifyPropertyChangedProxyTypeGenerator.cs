...
namespace Nextended.Core.DeepClone
{
    ...
    internal class INotifyPropertyChangedProxyTypeGenerator
    {
        ...
        public static Type GenerateProxy(Type type)
        {
            ...
            if (!typeof(INotifyPropertyChanged).IsAssignableFrom((Type) type))
            ...
            var props = Cloner.GetFastDeepClonerProperties(type).Where(x => x.CanRead && x.IsVirtual == true);
        ...
        private static void WrapMethod(IClonerProperty item, MethodBuilder raisePropertyChanged, TypeBuilder typeBuilder)
        {
