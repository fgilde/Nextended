...
namespace Nextended.Core.Helper
{
	...
	{
		private static Dictionary<string, List<CurrencyImportInformation>> cache = new Dictionary<string, List<CurrencyImportInformation>>();
            ...
            Currency sourceRateCurrency = null, bool returnAverageRate = false)
        {
            ...
            string cacheKey = $"{fromDate}-to-{toDate}-as-{sourceRateCurrency.IsoCode}-{returnAverageRate}";
        ...
        /// <param name="returnAverageRate">Wenn true, wird ein ergebnis zurückgegeben, in dem jede Währung nur einmal enthalten ist, und als kurs der Durchschnittswert </param>
        /// <returns></returns>
        private static IEnumerable<CurrencyImportInformation> GetCurrencyExchangeRateDataCore(DateTime fromDate, DateTime toDate, 
			Currency sourceRateCurrency = null, bool returnAverageRate = false)
        {
			...
			string xmlUrl = "http://www.ecb.europa.eu/stats/eurofxref/eurofxref-hist.xml";

			TimeSpan timeSpan = (DateTime.Now - fromDate);
			if (timeSpan.Days <= 90)
					...
					for (int i = 0; i < xmlReader.AttributeCount; i++)
					{
						...
						if (xmlReader.Name == "Cube")
						{
							...
							if (xmlReader.AttributeCount == 2)
							{
								...
								string currency = xmlReader.Value;

								//Kurs auslesen
								xmlReader.MoveToAttribute("rate");
								Decimal targetRate = decimal.Parse(xmlReader.Value.Replace(".", ",")); // Multiply to this Rate is Target currency
								if (date <= toDate && date >= fromDate)
								{
									result.Add(new CurrencyImportInformation(date,targetRate,Currency.Find(currency),sourceRateCurrency));
								}
							}
			...
			if(sourceRateCurrency.IsoCode != "EUR")
			{
				decimal gbpRate = result.First(ci => ci.Currency == sourceRateCurrency).Rate;

				foreach (var information in result)
				{
					decimal usdRate = information.Rate;

					decimal euro = 1 / gbpRate;
					decimal usd = euro * usdRate;
					decimal newRate = usd / 1;
					information.Rate = newRate; //  Multiply to this Rate is Target currency
				}
			}
			...
			if(returnAverageRate)
			{
				...
				{
					var averageRate = grouping.Select(information => information.Rate).Aggregate((arg1, arg2) => arg1 + arg2) / grouping.Count();
					var range = new DateRange(grouping.OrderBy(information => information.Date).First().Date, grouping.OrderBy(information => information.Date).Last().Date);
					averageResult.Add(new CurrencyImportInformation(range, averageRate, Currency.Find(grouping.Key), sourceRateCurrency));
				}
