...
public static class AceStepUiBuilderExtensions
{
        ...
        Action<AceStepUiOptions>? configure = null)
    {
        ...
        {
            // Match the UI: build the server from source at the pinned ref via the generic
            // github-repo helper (clone/refresh on the host + generated Dockerfile — older refs like
            // v0.1.4 ship none, and it must work with or without a tracked uv.lock, hence
            // `uv sync --frozen || uv sync`).
            api = appBuilder.AddResource(apiResource).WithGithubSource(options.ApiRepository, o =>
            {
                o.GitRef = options.ApiGitRef;
                o.CheckoutDirectory = Path.Combine(appBuilder.AppHostDirectory, "obj", "localai", apiName);
                o.DockerfileContent = $$"""
