...
internal static class EdgeFunctionRouter
{
    ...
    public static void GenerateRouter(string path, List<string> functionNames)
    {
        ...
        sb.AppendLine("  // Negative lookbehind (?<!\\.) ensures we don't accidentally match the");
        sb.AppendLine("  // 'serve' in 'Deno.serve(...)' — functions that already call Deno.serve");
        sb.AppendLine("  // directly would otherwise become 'Deno.Deno.serve(...)' and crash on start.");
        sb.AppendLine("  transformedCode = transformedCode.replace(");
        sb.AppendLine("    /(?<!\\.)\\bserve\\s*\\(/g,");
        sb.AppendLine("    'Deno.serve({ port: ' + port + ' }, '");
        sb.AppendLine("  );");
        sb.AppendLine();
        sb.AppendLine("  // For functions that already use Deno.serve(...), inject the port arg so the");
        sb.AppendLine("  // worker actually listens on the port the router expects. Matches:");
        sb.AppendLine("  //   Deno.serve(async (req) => ...   →   Deno.serve({ port: NNNN }, async (req) => ...");
        sb.AppendLine("  // and only at the start of an arg list (no existing options object).");
        sb.AppendLine("  transformedCode = transformedCode.replace(");
        sb.AppendLine("    /Deno\\.serve\\s*\\(\\s*(?!\\{)/g,");
        sb.AppendLine("    'Deno.serve({ port: ' + port + ' }, '");
        sb.AppendLine("  );");
        sb.AppendLine();
