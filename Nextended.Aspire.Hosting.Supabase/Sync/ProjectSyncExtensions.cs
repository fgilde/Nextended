...
public static class ProjectSyncExtensions
{
    ...
    /// <summary>
    /// Enables synchronization from an online Supabase project with legacy parameters.
    /// </summary>
    [Obsolete("Use the overload with SyncOptions parameter instead.")]
    public static IResourceBuilder<SupabaseStackResource> WithProjectSync(
        this IResourceBuilder<SupabaseStackResource> builder,
        string? projectRef,
        string? serviceKey,
        ...
        bool syncData)
    {
        var options = SyncOptions.StorageBuckets; // Always sync buckets
        if (syncSchema) options |= SyncOptions.Schema;
        if (syncData) options |= SyncOptions.Data;

        return builder.WithProjectSync(projectRef, serviceKey, options, null);
    }
}
