...
resource shares_volumes_supabase_storage_0 'Microsoft.Storage/storageAccounts/fileServices/shares@2024-01-01' = {
  name: take('sharesvolumessupabasestorage0-${uniqueString(resourceGroup().id)}', 63)
  properties: {
...
resource managedStorage_volumes_supabase_storage_0 'Microsoft.App/managedEnvironments/storages@2025-07-01' = {
  name: take('managedstoragevolumessupabasestorage${uniqueString(resourceGroup().id)}', 24)
  properties: {
    azureFile: {
      accountName: env_storageVolume.name
      accountKey: env_storageVolume.listKeys().keys[0].value
      accessMode: 'ReadWrite'
      shareName: shares_volumes_supabase_storage_0.name
    }
  }
  parent: env
}

output volumes_supabase_storage_0 string = managedStorage_volumes_supabase_storage_0.name
